...
public class HighlightSphere {
    ...
    public HighlightSphere(HighlightColor c) {
        this.color = c;
        this.groupID = UUID.randomUUID();
    }

    public static final ConcurrentHashMap<UUID, HighlightSphere> userConfigs = new ConcurrentHashMap<>();

    public static void setHighlightCenterAndRadius(UUID userId, double x, double y, double z, double radius, HighlightColor color, Level level) {
        userConfigs.compute(userId, (id, existingConfig) -> {
            if (existingConfig == null) {
                existingConfig = new HighlightSphere(color);
            }
        ...
        for (int dx = -intRadius; dx <= intRadius; dx++) {
            ...
            for (int dy = -intRadius; dy <= intRadius; dy++) {
                ...
                for (int dz = -maxDz; dz <= maxDz; dz++) {
                    ...
                    if (highlightRadius == 0 || (HighlightMemory.isBlockExposed(pos, level) && !HighlightMemory.contains(groupID, pos))) {
                        HighlightMemory.add(groupID, HighlightEntry.Whole(pos, this.color));
                    }
                }
            }
        }
    ...
    public void add(BlockPos placedPos, Level level) {
        ...
        if (HighlightMemory.isBlockExposed(placedPos, level)) {
            if (!HighlightMemory.contains(groupID, placedPos)) {
                HighlightMemory.add(groupID, HighlightEntry.Whole(placedPos, this.color));
            }
        } else {
            ...
            if (isWithinSphere(neighborPos)) {
                if (HighlightMemory.isBlockExposed(neighborPos, level)) {
                    if (!HighlightMemory.contains(groupID, neighborPos)) {
                        HighlightMemory.add(groupID, HighlightEntry.Whole(neighborPos, this.color));
                    }
                } else {
